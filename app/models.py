...
class User(UserMixin, db.Model):
    ...
    def generate_reset_password_token(self, expiration=900):
        s = Serializer(current_app.config['SECRET_KEY'], expiration)
    ...
    def follower_posts(self):
        return Post.query.join(Follow, Follow.follower_id == Post.author_id).filter(Follow.followed_id == self.id)
...
class Post(db.Model):
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            ...
            p = Post(body=forgery_py.lorem_ipsum.sentences(randint(2, 5)),
